...
void SignatureKnownAnswerTest(const char *key, const char *message, const char *signature)
{
	...
	VerifierFilter verifierFilter(verifier, nullptr, VerifierFilter::SIGNATURE_AT_BEGIN | VerifierFilter::THROW_EXCEPTION);
	...
	try
	{
		...
		StringSource(
			...
			new SignerFilter(
				rng,
				signer,
				new VerifierFilter(verifier, nullptr, VerifierFilter::THROW_EXCEPTION),
				true));
	}
	catch (...)
	{
