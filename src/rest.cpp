...
static bool rest_getutxos(HTTPRequest* req, const std::string& strURIPart)
{
    ...
    // check spentness and form a bitmap (as well as a JSON capable human-readable std::string representation)
    ...
    bitmap.resize((vOutPoints.size() + 7) / 8);
    {
        ...
        for (size_t i = 0; i < vOutPoints.size(); i++)
        {
            ...
            hits.push_back(hit);
            bitmapStringRepresentation.append(hit ? "1" : "0"); // form a binary std::string representation (human-readable for json output)
