...
CMessageHeader::MessageMagic netMagic = {0xf9, 0x73, 0xc9, 0xa7};

#define LUX_SEED_NONCE 0x0539a019ca550825ULL
...
class CSeederNode {
    ...
    void PushVersion() {
        ...
        uint64_t nLocalNonce = LUX_SEED_NONCE;
        int64_t nLocalServices = 0;
        CService myService;
        CAddress me(myService, ServiceFlags(NODE_NETWORK | NODE_WITNESS));
        BeginMessage("version");
        int nBestHeight = GetRequireHeight();
        std::string ver = "/lux-seeder:5.0.0/";
...
public:
    CSeederNode(const CService &ip, std::vector<CAddress> *vAddrIn)
        : vSend(SER_NETWORK, 0), vRecv(SER_NETWORK, 0), nHeaderStart(-1),
          ...
          you(ip, ServiceFlags(NODE_NETWORK | NODE_WITNESS)) {
