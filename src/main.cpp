...
static bool DisconnectTip(CValidationState& state, const CChainParams& chainparams)
{
    ...
    int64_t nStart = GetTimeMicros();
    {
        ...
        if (DisconnectBlock(block, state, pindexDelete, view, nullptr) != DISCONNECT_OK)
...
bool ActivateBestChain(CValidationState& state, const CChainParams& chainparams, const CBlock* pblock)
{
    ...
    do {
        ...
        while (true) {
            ...
            if (!ActivateBestChainStep(state, chainparams, pindexMostWork, pblock && pblock->GetHash(usePhi2) == pindexMostWork->GetBlockHash() ? pblock : nullptr))
                return false;
...
bool static LoadBlockIndexDB()
{
    ...
    if (!fLastShutdownWasPrepared && !GetBoolArg("-forcestart", false) && !GetBoolArg("-reindex", false) && (vSortedByHeight.size() != vinfoBlockFile[nLastBlockFile].nHeightLast + 1) && (vinfoBlockFile[nLastBlockFile].nHeightLast != 0)) {
        ...
        CValidationState state;
        ProcessNewBlock(state, chainparams, nullptr, &lastMetaBlock, &blockPos);
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                if (mapBlockIndex.count(hash) == 0 || (mapBlockIndex[hash]->nStatus & BLOCK_HAVE_DATA) == 0) {
                    CValidationState state;
                    if (ProcessNewBlock(state, chainparams, nullptr, &block, dbp))
                        nLoaded++;
                    if (state.IsError())
                        break;
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        if (ReadBlockFromDisk(block, it->second, nHeight, chainparams.GetConsensus())) {
                            ...
                            if (ProcessNewBlock(dummy, chainparams, nullptr, &block, &it->second)) {
                                nLoaded++;
