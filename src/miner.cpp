...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlockWithKey(CReserveKey& reservekey, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    return CreateNewBlock(scriptPubKey, fProofOfStake, pTotalFees, txProofTime, nTimeLimit);
}
...
bool CheckWork(const CChainParams &chainparams, CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        CValidationState state;
        if (!ProcessNewBlock(state, chainparams, NULL, pblock))
            return error("LUXMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { while (true) {
        if (chainparams.NetworkID() != CBaseChainParams::REGTEST) {
            // Busy-wait for the network to come online so we don't waste time mining
            // on an obsolete chain. In regtest mode we expect to fly solo.
            while (vNodes.empty())
            {
                printf("LUXMiner is waiting nodes...");
                MilliSleep(1000);
            }
        }
        ...
        bool fProofOfShake = false;
        std::unique_ptr<CBlockTemplate> pblocktemplate = BlockAssembler(chainparams).CreateNewBlockWithKey(reservekey, fProofOfShake);
        if (!pblocktemplate.get())
        {
            LogPrintf("LUXMiner stoped\n");
            return;
        }
        ...
        while (true)
        {
            ...
            while (true)
            {
                ...
                if (hash <= hashTarget)
                {
                    ...
                    CheckWork(chainparams, pblock, *pwallet, reservekey);
                    SetThreadPriority(THREAD_PRIORITY_LOWEST);
            ...
            if (vNodes.empty() && chainparams.NetworkID() != CBaseChainParams::REGTEST)
                break;
